...
def zipdir(archivename, basedir):
    ...
    with closing(ZipFile(archivename, "w", [ZIP_DEFLATED,allowZip64])) as z:
...
def my_zip(archivename, basedir, submission_type):
    '''
    ...
    '''
    assert os.path.isdir(basedir)
    # Here call zip in different ways:
    if submission_type == "resultOnly":
        print ("resultonly")
        # Keep xxx_valid.predict and xxx_test.predict files in the res/ directory, but not xxx_model.pickle
        with closing(ZipFile(archivename, "w", ZIP_DEFLATED,allowZip64=True)) as z:
            for root, dirs, files in os.walk(basedir):
                #NOTE: ignore empty directories
                if root == ".\public_data":
                    print ("sauter public_data")
                    absfn = os.path.join(root, "")
                    z.write(absfn)
                    continue
                for fn in files:
                    if fn[-7:]=='.pickle':
                    ...
                    if fn[-4:]!='.zip':
                        ...
                        zfn = absfn[len(basedir)+len(os.sep):] #XXX: relative path
                        z.write(absfn, zfn)
    elif submission_type == "modelOnly":
        # Keep xxx_model.pickle file in the res/ directory, but not xxx_valid.predict and xxx_test.predict files
        print("modelonly")
        with closing(ZipFile(archivename, "w", ZIP_DEFLATED,allowZip64=True)) as z:
            for root, dirs, files in os.walk(basedir):
                #NOTE: ignore empty directories
           ...
           # Do not keep anything in the res/ directory
        print("resonly")
        with closing(ZipFile(archivename, "w", ZIP_DEFLATED,allowZip64=True)) as z:
            for root, dirs, files in os.walk(basedir):
                #NOTE: ignore empty directories
